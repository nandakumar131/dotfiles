...
list_tasks() {
  source "$HOME/.config/sketchybar-top/colors.sh"
  local -a args=()
  local task_count=0
  ...
  sketchybar-top -m "${args[@]}"
}

# Function to toggle task popup in sketchybar
popup() {
  sketchybar-top --set "$NAME" popup.drawing="$1"
  sketchybar-top --set "$NAME" popup.background.color=$POPUP_BACKGROUND_COLOR
  sketchybar-top --set "$NAME" popup.background.corner_radius=10
}
...
update() {
  ...
  if [[ $pending_task_count == 0 ]]; then
    sketchybar-top --set $NAME label.drawing=off
  else
    local label
    ...
    sketchybar-top --set $NAME label="$label" \
      label.drawing=on
  fi
}
