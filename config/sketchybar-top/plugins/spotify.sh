...
repeat () 
{
  ...
  if [ "$REPEAT" = "false" ]; then
    sketchybar-top -m --set spotify.repeat icon.highlight=on
    osascript -e 'tell application "Spotify" to set repeating to true'
  else 
    sketchybar-top -m --set spotify.repeat icon.highlight=off
...
shuffle () 
{
  ...
  if [ "$SHUFFLE" = "false" ]; then
    sketchybar-top -m --set spotify.shuffle icon.highlight=on
    osascript -e 'tell application "Spotify" to set shuffling to true'
  else 
    sketchybar-top -m --set spotify.shuffle icon.highlight=off
